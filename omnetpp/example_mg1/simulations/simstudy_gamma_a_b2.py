...
class model(bootstrapcbModel):
    ...
    class distribution(rv_continuous):     
        def _cdf(self, x, a, b):
            return gamma.cdf(x, a, scale=b/a) 
        def _logcdf(self, x, a, b):
            return gamma.logcdf(x, a, scale=b/a) 
        def _pdf(self, x, a, b):
            return gamma.pdf(x, a, scale=b/a) 
        def _logpdf(self, x, a, b):
            return gamma.logpdf(x, a, scale=b/a) 
        def _ppf(self, q, a, b):
            return gamma.ppf(q, a, scale=b/a) 
        def _rvs(self, a, b, size=None, random_state=None):
            return gamma.rvs(a, scale=b/a, size=size, random_state=random_state)
...
# init analysis
shape, scale = 1, 0.8
theta_0 = [shape, shape*scale] 
path = "datasets/"
sim_runs_dataset = "tmp.csv"
bootstrapcb_dataset = "simstudy_gamma_a_b2.json"
simstudy_dataset = "simstudy_gamma_a_b2.txt"
...
# results_test.data_per_run = np.array([{'endog': gamma.rvs(a=shape, scale=scale, size=50)} for _ in range(500)])
########################################################################
